...
public class CassandraConf implements Serializable {
    ...
    public CassandraConf(Map<String, Object> conf) {
        ...
        this.username = (String) ClientUtils.get(conf, CASSANDRA_USERNAME, null);
        this.password = (String) ClientUtils.get(conf, CASSANDRA_PASSWORD, null);
        this.keyspace = get(conf, CASSANDRA_KEYSPACE);
        this.consistencyLevel = ConsistencyLevel.valueOf((String) ClientUtils.get(conf, CASSANDRA_CONSISTENCY_LEVEL, ConsistencyLevel.ONE.name()));
        this.nodes    = ((String) ClientUtils.get(conf, CASSANDRA_NODES, "localhost")).split(",");
        ...
        this.retryPolicyName = (String) ClientUtils.get(conf, CASSANDRA_RETRY_POLICY, DefaultRetryPolicy.class.getSimpleName());
